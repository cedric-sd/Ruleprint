...
| `fixture-express-api.ruleprint.json` | generated: `ruleprint scan` over `../fixture-express-api` (reading its `ruleprint.lock`, so every rule is approved) with a fixed clock and no git; `pnpm check:golden` compares, `pnpm update:golden` refreshes |
