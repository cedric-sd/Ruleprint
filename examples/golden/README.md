...
| `fixture-express-api.ruleprint.json` | generated: `ruleprint scan` over `../fixture-express-api` (reading its `ruleprint.lock`; the declared rule with an id is fused with the shipping test, the one without evidence is `orphan`, the rest are approved) with a fixed clock and no git; `pnpm check:golden` compares, `pnpm update:golden` refreshes |
