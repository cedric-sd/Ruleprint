...
describe('approveProject()', () => {
  it('--all writes the lock and a ruleprint.json where everything is approved', async () => {
    ...
    expect(before.changes.map((c) => c.kind)).toEqual(new Array<string>(21).fill('added'));
    ...
    expect(Object.keys(readLock(dir)?.rules ?? {})).toHaveLength(21);
    ...
    expect(written.rules.filter((r) => r.status === 'approved')).toHaveLength(20);
  ...
  it('approves only the given ids', async () => {
    ...
    const after = await scanProject(dir, { now: NOW, git: false });
    expect(after.changes).toHaveLength(19);
  });
