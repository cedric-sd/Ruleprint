...
export interface NormalizeOptions {
  /** A node to replace by `placeholder` in the output (e.g. a test title). */
  readonly replace?: Node;
  readonly placeholder?: string;
}

/**
 * Normalised form of a subtree (ADR-0005): the S-expression of its named nodes, comments
 * dropped, strings by content, a bare arrow parameter written like a parenthesised one, and
 * locally declared identifiers alpha-renamed in order of first appearance. Formatting and local
 * renames leave it unchanged; literals, free identifiers, property names and structure change it.
 */
export function normalizeNode(root: Node, options: NormalizeOptions = {}): string {
  ...
  serialize(
    root,
    {
      ...
      placeholder: options.placeholder ?? '<replaced>',
      locals,
      renamed: new Map(),
    },
    out,
...
/** Every identifier bound inside the subtree: parameters, declarations, patterns, catch, loops. */
...
function serialize(node: Node, ctx: Context, out: string[]): void {
  ...
  if (ctx.replaceId !== undefined && node.id === ctx.replaceId) {
    out.push(ctx.placeholder);
    return;
  }
  if (node.type === 'string') {
